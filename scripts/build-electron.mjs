...
const defaults=JSON.parse(readFileSync('build/google-oauth.json','utf8'));
const raw=existsSync(configPath)?JSON.parse(readFileSync(configPath,'utf8')):{};
if(raw.web)throw new Error('Use a Desktop app OAuth configuration, not a Web application client.');
const config=raw.installed??raw;
const clientId=process.env.OWL_GOOGLE_CLIENT_ID??config.clientId??config.client_id??defaults.clientId;
const clientSecret=process.env.OWL_GOOGLE_CLIENT_SECRET??config.clientSecret??config.client_secret??'';
