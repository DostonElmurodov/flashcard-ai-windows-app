...
function Harness(){
 const [snapshot,setSnapshot]=useState(data),[review,setReview]=useState(false),[preview,setPreview]=useState(false);
 publish=()=>setSnapshot({...data});
 return <WorkspaceBridgeProvider bridge={bridge}><main style={{maxWidth:1050,margin:'30px auto',padding:'0 24px'}}><h1>Your preferences</h1><p className="muted">A learning routine that feels like you.</p><Settings key={JSON.stringify(snapshot.settings)} settings={snapshot.settings} onChanged={async()=>publish()} notify={()=>{}}/><button className="button" onClick={()=>setReview(true)}>Open review</button><button className="button" onClick={()=>setPreview(true)}>Open AI preview</button>{preview&&<CreateSet snapshot={snapshot} deck={snapshot.decks[0]} cardsOnly onSaved={()=>{}} notify={()=>{}}/>}{review&&<Review snapshot={snapshot} onClose={()=>setReview(false)} onChanged={async()=>publish()}/>}</main></WorkspaceBridgeProvider>;
}
