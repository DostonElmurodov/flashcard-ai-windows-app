...
export default function TitleBar(){
 ...
 return <header className="window-titlebar"><div className="window-title"><Owl size={19}/><span>Owl AI</span></div><nav className="window-menus" aria-label="Application menu">{names.map((name,index)=><button key={name} ref={node=>{buttons.current[index]=node;}} aria-haspopup="menu" aria-expanded={opened===name} onClick={()=>void open(index)} onKeyDown={event=>{if(event.key==='ArrowRight'||event.key==='ArrowLeft'){event.preventDefault();buttons.current[(index+(event.key==='ArrowRight'?1:2))%3]?.focus();}else if(event.key==='ArrowDown'){event.preventDefault();void open(index);}}}>{name}</button>)}</nav></header>;
}
