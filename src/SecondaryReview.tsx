...
export default function SecondaryReview({wordId,preview,language,accessRevision}:{wordId?:string;preview?:{word:string;native:string;learning:string};language:string;accessRevision:string}){
 const owl=useOwl();
 ...
 useEffect(()=>{
  ...
  const request=preview?owl.previewTranslation(preview.word,preview.native,preview.learning):owl.reviewTranslation(wordId!);
  void request.then(value=>{if(active)setContent(value);}).catch(reason=>{if(active)setError(errorMessage(reason));});
  return()=>{active=false;};
