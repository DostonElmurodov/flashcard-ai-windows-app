...
export class Store {
 ...
 static async open(path:string):Promise<Store>{
  ...
  db.run('PRAGMA foreign_keys=ON'); const store=new Store(db,path);store.persist();return store;
 }
 ...
 saveDeck(input:Partial<Deck>&{name:string}):Deck {
  ...
  const existing=input.id?this.snapshot().decks.find(x=>x.id===input.id):undefined;
 ...
 deleteDeck(id:string){this.transaction(()=>{this.db.run('DELETE FROM reviews WHERE word_id IN (SELECT id FROM words WHERE deck_id=?)',[id]);this.db.run('DELETE FROM words WHERE deck_id=?',[id]);this.db.run('DELETE FROM decks WHERE id=?',[id]);});}
