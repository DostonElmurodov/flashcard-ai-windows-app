...
function context(workspace:TranslationWorkspace,wordId:string){
 const {settings,words,decks}=workspace.store.snapshot();
 ...
 const native=canonicalLanguage(word.nativeLanguage??deck.nativeLanguage),learning=canonicalLanguage(word.learningLanguage??deck.learningLanguage);
 const secondary=secondaryReviewLanguage(settings.secondaryReviewLanguage,native??'');
 ...
 const input:ReviewTranslationRequest={word:word.word.trim().normalize('NFC'),native_language:native,learning_language:learning,secondary_language:secondary};
...
export class ReviewTranslations {
 ...
 async get(wordId:string):Promise<ReviewTranslation>{
  const workspace={...this.current()},{input,key}=context(workspace,wordId);
  ...
  const pending=(async()=>{
   ...
   if(current.store!==workspace.store||current.scope!==workspace.scope||context(current,wordId).key!==key)
