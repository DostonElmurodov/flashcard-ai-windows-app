...
export class Api {
 ...
 async request<T>(path:string,method='POST',body?:unknown):Promise<T>{if(!this.session)throw new Error('Sign in to your Owl AI account to continue.');const generation=this.generation;if(new Date(this.session.access_token_expires_at).getTime()<Date.now()+30000)await this.refresh();if(generation!==this.generation||!this.session)throw new Error('Your account session changed.');let response=await this.send(path,method,body,this.session.access_token);if(response.status===401){await this.refresh();if(generation!==this.generation||!this.session)throw new Error('Your session expired.');response=await this.send(path,method,body,this.session.access_token);}if(generation!==this.generation)throw new Error('Your account session changed.');return this.result<T>(response);}
